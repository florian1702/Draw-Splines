...
public class Gauss {
	...
	/**
	 * Loest ein lineares GLS mit mehreren rechten Seiten 
	 * nach dem Gaussverfahren.
	 * 
	...
	public static float[][] rueckwaertsSubstitutionMehrereGLS(float[][] m) {
		int n = m.length;
		int nGls = m[0].length - n;
		float[][] x = new float[n][nGls];
		for (int k = 0; k < nGls; k++) {
			x[n - 1][k] = m[n - 1][n + k] / m[n - 1][n - 1];
			for (int i = n - 2; i > -1; i--) {
				float tmp = m[i][n + k];
	...
	public static float[][] dreiecksElimination(float[][] m) {
		float[][] r = Matrix.cloneMatrix(m);
		int n = r.length;
		// --- fuer alle zu eliminierenden Spalten
		for (int j = 0; j < n - 1; j++) {
			double max = Math.abs(r[j][j]);
	...
	/**
	 * Berechnet die Determinante einer quadratischen Matrix. Es wird zunaechst eine Dreieckselimination durchgefuehrt.
	 * Die Determinante berechnet sich als Produkt der Hauptdiagonalelemente.
	 * 
	 * @param m n*n Matrix
	...
	/**
	 * Berechnet das Hadamardsche Konditionsmass Kh < 0.01 schlechte Kondition Kh > 0.1 gute Kondition
	 * 
